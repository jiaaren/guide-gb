...
## Code produced by GUIDE 45.2 on 11/7/25 at 23:39
predicted <- function(){
 if(!is.na(nox) & nox <= 0.715500000000 ){
   if(!is.na(dis) & dis <= 2.50055000000 ){
     if(!is.na(tax) & tax <= 420.000000000 ){
       ...
       if(!is.na(indus) & indus <= 19.9950000000 ){
         nodeid <- 18
         predict <- -0.194927572023
       } else {
         nodeid <- 19
         predict <- -0.144241316408
       }
     }
   } else {
...
##
##
## If desired, replace "predicted2" with name of file containing new data
...
newdata <- read.table("predicted2",header=TRUE,colClasses="character")
