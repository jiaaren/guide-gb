...
model <- guide_gb(train, trainy, guide_path=guide_path, config_path=config_path,
                   ...
                   fit_pred_exact = FALSE, val_x = test, val_y = testy, early_stop_rounds = 50, eta=0.1)
end <- Sys.time()
end - start
...
# calculate test loglik
pred_logodds <- predict(model, test, type="link")
loglik2(actual = testy, log_odds = pred_logodds)
model$fit$err_val
