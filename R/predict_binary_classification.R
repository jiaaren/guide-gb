...
get_iteration_gradients_classifier <- function(fit, x, pred_func, n_trees = NULL) {
  # n_trees passed here cannot be a vector, hence max
  n_trees <- max(n_trees %||% fit$iterations)
  # classifier references node
  results <- lapply(fit$trees[1:n_trees], function(f) pred_func(x, f)$node)
  mapped_results <- mapply(map_logodds, fit$tree_maps[1:n_trees], results, SIMPLIFY = TRUE)
  sweep(mapped_results, 2, fit$eta[1:n_trees], `*`)
}

make_classifier_prediction <- function(fit, x, pred_func, n_trees) {
  mult <- get_iteration_gradients_classifier(fit, x, pred_func, n_trees)
  if (length(n_trees) > 1) {
